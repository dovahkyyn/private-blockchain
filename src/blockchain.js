...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
            try{
                ...
                if(valid.length == 0){
                    ...
                    throw Error("Error on validating blocks");
                }
            ...
            try{
                ...
                if(currentTime - messageTime < 300){
                    if(bitcoinMessage.verify(message, address, signature)){
                        ...
                        throw Error("Not able to verify message");
                    }
                }else{
                    throw Error("More than 5 min passed since the message was created");
                }
            }catch(err){
                reject(err)
            }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = self.chain.find((currentBlock) => { currentBlock.hash == hash });
            resolve(block);
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async (block) => {
                await block.validate( () => {}, (error) => { errorLog.push(error) } );

                if(block.height > 0 && block.previousBlockHash != self.chain[block.height - 1].hash){
                    errorLog.push('Previous block hash do not match');
                }
            });
